...
void WorldBorderVisuals::initialise_world_border(float world_radius)
{
	...
	ShaderManager::get_instance().load_shader(WORLD_BORDER_SHADER_TAG, WORLD_BORDER_SHADER_PATH);
	set_world_radius(world_radius);
}
void WorldBorderVisuals::draw_world_border(const glm::vec3& player_position)
{
	std::shared_ptr<Shader> world_border_shader = ShaderManager::get_instance().get_shader(WORLD_BORDER_SHADER_TAG);
...
void WorldBorderVisuals::set_world_radius(const float world_radius)
{
	...
	ShaderManager::get_instance().get_shader(WORLD_BORDER_SHADER_TAG)->set_float("world_radius", world_radius);
}